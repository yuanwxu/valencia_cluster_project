...
we should correct the inconsistent formating. There is also a data entry error in one of the labels in the
`.fas` file, this has been fixed; otherwise the `treedater` wrapper defined below would report error.
```{r epi_info}
epi_info <- read_excel("Data/epi_info.xlsx")

# === Correction for data inconsistency ===
...
tdaterAnalysis <- function(file, sqlen, epi_info, ...){
  ...
  if(anyNA(sts$sts) || length(sts)>1){
    ...
    dtr <- dater(tre, mf$sts, sqlen, strict = TRUE, estimateSampleTimes = mf$sts.df)
  }
  else{
    dtr <- dater(tre, sts$sts, sqlen, strict=TRUE, ...)  
  }
  dtr
}
```
...
library(treedater)
# Treedater analysis
dtr <- tdaterAnalysis(file = "MLTree/noMLRA/constSite/run_alignment_CL01.fas.treefile", sqlen = 4444, epi_info)
plot(dtr) 
```
...
set.seed(1)
record <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 4000, thinning = 10)
```

The combined phylogenetic and transmission tree and the consensus transmission tree obtained from 
the posterior:
```{r}
plotCTree(record[[400]]$ctree)
cons_tree <- consTTree(record)
...
to `wiwMedTree`, we need a list of MRCI depths matrices corresponding to the trees in `record`, so we use
...
plotCTree(record[[out$median]]$ctree) 
plotTTree(extractTTree(record[[out$median]]$ctree), w.shape, w.scale)
```

The median transmission tree looks similar to the consensus transmission tree, except that the group that was
infected earlier in the consensus tree now gets infected later --- this is not very suprising given that the
interval between the two infections is relatively small in both the consensus and the median tree, and so there
is uncertainty which happened earlier.
...
tibble(unsamp = sapply(record, function(x) getNumberUnsampled(x$ctree))) %>%
  ggplot(aes(unsamp)) +
  geom_bar()
```
...
prob_unsamp_src(record)
```

We run a longer `transphylo` simulation in order to get some statistics for the basic reproduction number $R_0$.
We see that both the mean and median of $R_0$ are less than 1.
```{r basic_reprod_num, results="hide"}
record <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 10000, thinning = 10)
plot(map_dbl(record, "off.r"), ylab='Basic reproduction number R', xlab='MCMC iterations',type='l')
```
```{r}
summary(map_dbl(record, "off.r")[500:1000])
```


### Repeat for some other clusters ###
```{r, include=FALSE}
dtr <- tdaterAnalysis(file = "MLTree/noMLRA/constSite/run_alignment_CL03.fas.treefile", sqlen = 4429, epi_info)
plot(dtr) 
dtr$edge.length <- with(dtr, ifelse(abs(edge.length)<1e-5, edge.length+0.1, edge.length))
plot(dtr)
```
...
plotCTree(record_CL03[[400]]$ctree)
cons_tree <- consTTree(record_CL03)
plotTTree(cons_tree, w.shape, w.scale)
```

```{r, include=FALSE}
dtr <- tdaterAnalysis(file = "MLTree/noMLRA/constSite/run_alignment_CL04.fas.treefile", sqlen = 4423, epi_info)
plot(dtr) 
dtr$edge.length <- with(dtr, ifelse(abs(edge.length)<1e-5, edge.length+0.1, edge.length))
plot(dtr)
```
...
plotCTree(record_CL04[[400]]$ctree)
cons_tree <- consTTree(record_CL04)
plotTTree(cons_tree, w.shape, w.scale)
```

```{r, include=FALSE}
dtr <- tdaterAnalysis(file = "MLTree/noMLRA/constSite/run_alignment_CL07.fas.treefile", sqlen = 4427, epi_info)
plot(dtr) 
```
```{r, include=FALSE, results="hide"}
ptre <- ptreeFromPhylo(dtr, dateLastSample = max(dtr$sts))
record_CL07 <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 10000, thinning = 10)
plotCTree(record_CL07[[400]]$ctree)
cons_tree <- consTTree(record_CL07)
plotTTree(cons_tree, w.shape, w.scale)
```

```{r, include=FALSE}
dtr <- tdaterAnalysis(file = "MLTree/noMLRA/constSite/run_alignment_CL10.fas.treefile", sqlen = 4421, epi_info)
plot(dtr) 
dtr$edge.length <- with(dtr, ifelse(abs(edge.length)<1e-5, edge.length+0.1, edge.length))
plot(dtr)
```
...
plotCTree(record_CL10[[400]]$ctree)
cons_tree <- consTTree(record_CL10)
plotTTree(cons_tree, w.shape, w.scale)
```

Some other clusters of relatively large size were studied, following the same procedure for cluster `CL01`, and the posterior probability of unsampled source case is shown in this table. We see that it is almost certain that
`CL01` was infected by someone outside of the cluster, whereas it is very likely that `CL03` and `CL10` were infected by someone within the cluster. It is not certain about this for `CL04` and `CL07`.

```{r unsamp_src_prob, echo=FALSE}
tibble(cluster = stringr::str_c("CL", c("01","03","04","07","10")),
           prob = map_dbl(list(record, record_CL03, record_CL04, 
...
which can be extracted from the output.

```{r reprod_num, echo=FALSE}
basic_reprod_num <- function(record) summary(map_dbl(record, "off.r")[500:1000])

tibble(cluster = stringr::str_c("CL", c("01","03","04","07","10")),
           R_0 = map(list(record, record_CL03, record_CL04, 
