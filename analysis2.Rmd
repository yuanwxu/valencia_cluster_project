...
ptre <- ptreeFromPhylo(dtr, dateLastSample = max(dtr$sts))
record_CL03 <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 4000, thinning = 10)
...
```{r, include=FALSE, results="hide"}
ptre <- ptreeFromPhylo(dtr, dateLastSample = max(dtr$sts))
record_CL04 <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 4000, thinning = 10)
...
```{r, include=FALSE, results="hide"}
ptre <- ptreeFromPhylo(dtr, dateLastSample = max(dtr$sts))
record_CL07 <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 4000, thinning = 10)
...
```{r, include=FALSE, results="hide"}
ptre <- ptreeFromPhylo(dtr, dateLastSample = max(dtr$sts))
record_CL10 <- inferTTree(ptre, w.shape, w.scale, ws.shape, ws.scale, mcmcIterations = 4000, thinning = 10)
...
data.frame(cluster = stringr::str_c("CL", c("01","03","04","07","10")),
